...
pip3 install Pillow PyJWT cryptography --quiet --break-system-packages 2>/dev/null \
  || pip3 install Pillow PyJWT cryptography --quiet 2>/dev/null \
  || { echo "Error: pip install failed. Try: pip3 install Pillow PyJWT cryptography"; exit 1; }
